...
var log_file = fs.createWriteStream(__dirname + '/logs/debug_knownBots_banner.log', { flags: 'a' });
...
async function main() {
  setTerminalTitle("Known Bot Banner")
  wentLive = []
  stillRunning = true
  try {
    con.on('error', (err) => {
    ...
    data = await con.promise().query(`select ban_username from ${process.env.DB_BotListTable}`)
    data[0].forEach((record) => {
...
function getNextRunTime() {
  var dt = new Date()
  dt.setHours(dt.getHours() + (process.env.TIME_RUN_OCCURANCE_BOTLIST / 3600000))
...
}, process.env.TIME_RUN_OCCURANCE_BOTLIST);
